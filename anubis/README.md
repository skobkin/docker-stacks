...
- **The dist blocks Alibaba, Huawei, and Tencent Cloud by CIDR.** The dist imports the Anubis-curated `(data)/crawlers/alibaba-cloud.yaml`, `huawei-cloud.yaml`, and `tencent-cloud.yaml` denylists. These are flat `remote_addresses` lists of the providers' IP ranges, so the deny fires without a Thoth subscription. If your backend or a trusted partner is hosted on one of those networks, remove the corresponding import from `policy.yml` and add an explicit `ALLOW` rule above for the affected ranges. To ban additional networks, use the `iplist2rule` workflow documented in the dist's header comment.
